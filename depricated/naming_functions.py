...
def select_latest_version(df):
    ...
    '''
    df["TABLE_NAME_Backup"] = df["TABLE_NAME"]
    df["TABLE_NAME"] = df.apply(remove_part, axis = 1, args = ("date",))
    df["FORMATTED_TABLE_NAME"] = df.apply(remove_part, axis = 1, args = ("version",))
    
    df["version"] = df["TABLE_NAME"].apply(get_version)
    # UPDATE 20220207 to include schema in duplicate assessment
    df["TABLE_NAME"] = df["TABLE_NAME_Backup"]

    df = df.sort_values('version').drop_duplicates(["FORMATTED_TABLE_NAME", "TABLE_SCHEMA"], keep='last')
...
def select_latest_date(df):
    ...
    Returns:
        df (DataFrame): table register including only latest versions
    '''
    df["FORMATTED_TABLE_NAME"] = df.apply(remove_part, axis = 1, args = ("date",))
    df["date"] = df["TABLE_NAME"].apply(get_date)
